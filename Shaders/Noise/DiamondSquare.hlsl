...
int DATA_SIZE = _terrainPoints + 1; // must be a power of two plus one e.g. 33, 65, 128, etc
            ...
            data[0, 0] = data[0, DATA_SIZE - 1] = data[DATA_SIZE - 1, 0] =
              data[DATA_SIZE - 1, DATA_SIZE - 1] =
_seed;

double h = _roughness; //the range (-h -> +h) for the average offset - affects roughness
Random r = newRandom();//for the new value in range of h
                                    //side length is distance of a single square side
                                    //or distance of diagonal in diamond

            for (
int sideLength = DATA_SIZE - 1;
...
int halfSide = sideLength / 2;
                ...
                for (
int x = 0;x < DATA_SIZE - 1; x += sideLength) {
                    for (
int y = 0;y < DATA_SIZE - 1; y += sideLength) {
...
h;
                ...
                for (
int x = 0;x < DATA_SIZE - 1; x += halfSide) {
                    //and y is x offset by half a side, but moved by
                    //the full side length
                    //NOTE: if the data shouldn't wrap then y < DATA_SIZE
                    //to generate the far edge values
                    for (
int y = (x + halfSide) % sideLength;y < DATA_SIZE - 1; y += sideLength) {
